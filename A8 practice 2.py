...
def contains_duplicate(words):
    ...
    :param words: A list of anything
    :return: True if any two entries in the list are identical, otherwise False
    """
    for index_1 in range(len(words) - 1):
        for index_2 in range(index_1+1, len(words)):
            if words[index_1] == words[index_2]:
                return True
    return False
...
def list_to_string(numbers):
    ...
    if numbers == []:
        return '[]'
    string_of_list = '['
    string_of_list += str(numbers[0])
    for index in range(1, len(numbers)):
        string_of_list += ',' + str(numbers[index])
    string_of_list += ']'
...
def main():
    ...
    print("Testing curve_scores([50, 70, 90]). The expected result is [60, 80, 100], the actual result is",
          curve_scores([50, 70, 90]))
    print("Testing curve_scores([75, 100, 85]). The expected result is [75, 100, 85], the actual result is",
          curve_scores([75, 100, 85]))
    print()
    print("Testing contains_duplicate")
    print("Testing contains_duplicate(['a', 'b', 'c', 'd']). The expected result is False, the actual result is",
          contains_duplicate(['a', 'b', 'c', 'd']))
    print("Testing contains_duplicate(['a', 'b', 'c', 'b']). The expected result is True, the actual result is",
          contains_duplicate(['a', 'b', 'c', 'b']))
    print()
    print("Testing list_to_string")
    print("Testing list_to_string([1, 2, 3, 4]). The expected result is [1, 2, 3, 4], the actual result is",
    ...
    print("Testing binary_search_for_matching_string('hello',['hello', 'there', 'sir']). The expected result is hello,"
          " the actual result is", binary_search_for_matching_string('hello', ['hello', 'there', 'sir']))
    print("Testing binary_search_for_matching_string('missing', ['key', 'not', 'present']). The expected result is "
          "None, the actual result is", binary_search_for_matching_string('missing', ['key', 'not', 'present']))
