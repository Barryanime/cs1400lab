...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title="Graphics Window",
        ...
        tk.Canvas.__init__(self, master, width=width, height=height, bd=0, highlightthickness=0)
        ...
        self.mouseX = None
        self.mouseY = None
        self.currentMouseX = None
        self.currentMouseY = None
    ...
    def getCurrentMouse(self):
        """Return Point representing current mouse position."""
        #        _root.update()
        #        x = self.winfo_pointerx() - self.winfo_x()
        #        y = self.winfo_pointery() - self.winfo_y()
        #        print("Root pointerxy: ",self.winfo_pointerxy()," root: ", self.winfo_x(),self.winfo_y())
        #        return Point(x,y)

        if self.currentMouseX != None and self.currentMouseY != None:
            worldX, worldY = self.toWorld(self.currentMouseX, self.currentMouseY)
            if worldX < 0:
                worldX = 0
            if worldX > self.getWidth() - 1:
                worldX = self.getWidth() - 1
            if worldY < 0:
                worldY = 0
            if worldY > self.getHeight() - 1:
                worldY = self.getHeight() - 1
...
class GraphicsObject:
    ...
    def setFill(self, color):
        ...
        tcl_color = color_rgb(color[0], color[1], color[2])
        self._reconfig("fill", tcl_color)

    def setFill(self, red, green, blue):
        """Set interior color to color"""
        tcl_color = color_rgb(red, green, blue)
        self._reconfig("fill", tcl_color)
...
class Image(GraphicsObject):
    ...
    def getPixelRed(self, x, y):
        """Returns the red component of a pixel color for pixel (x,y)
        r,g,b are in range(256)
    ...
    def getPixelGreen(self, x, y):
        """Returns the green component of a pixel color for pixel (x,y)
        r,g,b are in range(256)
    ...
    def getPixelBlue(self, x, y):
        """Returns the blue component of a pixel color for pixel (x,y)
        r,g,b are in range(256)
    ...
    def setPixel(self, x, y, color):
        ...
        if type(color) is list:
            color = color_rgb(clamp(color[0]), clamp(color[1]), clamp(color[2]))
            self.img.put("{" + color + "}", (x, y))
        else:
            self.img.put("{" + color + "}", (x, y))

    def setPixelRed(self, x, y, new_color):
        """Sets the red part of pixel (x,y) to the given color

        """
        old_color = self.getPixel(x, y)
        if type(old_color) is list:
            color = color_rgb(clamp(new_color), clamp(old_color[1]), clamp(old_color[2]))
            self.img.put("{" + color + "}", (x, y))
        else:
            self.img.put("{" + old_color + "}", (x, y))

    def setPixelGreen(self, x, y, new_color):
        """Sets the green part of pixel (x,y) to the given color

        """
        old_color = self.getPixel(x, y)
        if type(old_color) is list:
            color = color_rgb(clamp(old_color[0]), clamp(new_color), clamp(old_color[2]))
            self.img.put("{" + color + "}", (x, y))
        else:
            self.img.put("{" + old_color + "}", (x, y))

    def setPixelBlue(self, x, y, new_color):
        """Sets the blue part of pixel (x,y) to the given color

        """
        old_color = self.getPixel(x, y)
        if type(old_color) is list:
            color = color_rgb(clamp(old_color[0]), clamp(old_color[1]), clamp(new_color))
            self.img.put("{" + color + "}", (x, y))
        else:
            self.img.put("{" + old_color + "}", (x, y))
